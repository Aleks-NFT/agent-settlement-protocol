...
pub fn handler(ctx: Context<Settle>) -> Result<()> {
    ...
    // 1. Читаем settle-цену с Pyth (on-chain, без параметров от агента)
    ...
    // 2. Считаем fee (с агента — USDC уже у него после execute)
    ...
    // 3. P&L (informational — для события и аналитики)
...
pub struct TradeSettled {
    ...
    pub agent:           Pubkey,
    pub fill_price:      u64,
    pub settle_price:    u64,   // ← новое поле
    pub amount:          u64,
    pub fee:             u64,
    pub fee_bps:         u16,
    pub payout_estimate: u64,   // ← новое поле (P&L для аналитики)
...
pub struct Settle<'info> {
    ...
    // vault нужен только для чтения — is_closed уже true после execute, это OK
    #[account(
    ...
    // fee источник — USDC уже у агента
    #[account(
        mut,
    ...
    /// CHECK: Pyth feed — magic и staleness валидируются вручную
